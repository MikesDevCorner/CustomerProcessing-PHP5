Application.ControllerUserGridAuswahl = function()
{
    var controller = this;
    ...
    this.Init = function()
    {
        ...
        Ext.getCmp('gridtitle').setValue("<b>&nbsp;User</b>");
    }
    ...
    this.SetListeners = function()
    {
        Ext.getCmp('btn_add').on('click', function() {
            ...
            userForm.Controller.enableAllChilds();
            userForm.Controller.enableAllToolbarButtons();
            userForm.getForm().reset();
            Ext.getCmp('formheaddisplay').setValue("<b style='color:red;'>NEUER USER, UNGESPEICHERT</b>");
            Ext.getCmp('id_user').setValue('0');
        ...
        Ext.getCmp('btn_delete').on('click', function() {
            if(controller.tmpComponent.getSelectionModel().hasSelection())
            {
                Ext.Msg.show({
                   title:'Löschen',
                   msg: 'Der User Nr. '+controller.tmpComponent.getSelectionModel().getSelected().data.id+', '+controller.tmpComponent.getSelectionModel().getSelected().data.nachname+' wird gelöscht.<br/><br/>Soll mit der Aktion fortgefahren werden?',
        ...
        this.tmpComponent.on('rowclick', function(grid,rowIndex){
            ...
            userForm.getForm().load({
                ...
                method: 'POST',
                params: {cmd: 'UserLoad', id_user: grid.getStore().getAt(rowIndex).id}
            });
        });
