Application.ControllerPartnerFormPanel = function()
{
    var controller = this;
    ...
    this.SetListeners = function()
    {
        ...
        savebtn.on('click', function() {
            ...
            controller.tmpComponent.getForm().submit({
                ...
                success: function(response,options) {
                    Ext.example.msg('Erfolg', 'Partner gespeichert.');
                ...
                method: 'POST',
                params: {cmd: 'PartnerSave'}
            });
        });

        Ext.getCmp('btn_abbrechen').on('click', function() {
            Ext.getCmp('formheaddisplay').setValue('');
            var id_partner_load = Ext.getCmp('id_partner').getValue();
            if(id_partner_load != '0')
            {
                controller.tmpComponent.getForm().load({
                    url: 'index.php',
                    success: function(response,options) {
                        Ext.example.msg('Status', 'Partner wurde zurückgesetzt.');
                    },
                    failure: function() {
                    ...
                    params: {cmd: 'PartnerLoad', id_partner: id_partner_load}
                });
            }
            else
            {
        ...
        Ext.getCmp('btn_drucken').on('click', function() {
            //Ext.example.msg('Status', 'Partner ausdrucken');
